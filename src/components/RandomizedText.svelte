<script lang="ts">
  ...
  // biome-ignore lint/correctness/useHookAtTopLevel: this is a component from magicui svelte
  let view = useInView(
    // biome-ignore lint/style/noNonNullAssertion: this is a component from magicui svelte
  ...
  function normalizeStyleName(property: string) {
    ...
    return property.replace(/-([a-z])/g, (_, character: string) =>
  ...
  const expoOut = (time: number) => (time === 1 ? 1 : 1 - 2 ** (-10 * time));
  ...
  const segmentCount = $derived(
    tokens.reduce(
      (count, token) => count + (token.kind === "segment" ? 1 : 0),
      0,
    ),
  );
...
<MotionComponent
  bind:ref={element}
  class={cn("wrap-break-word whitespace-pre-line", className)}
  style={rootStyle}
  initial="hidden"
